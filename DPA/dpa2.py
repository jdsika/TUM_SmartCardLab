...
def preprocessing(iTraces):
    ...
    print "Starting Preprocessing: %d samples -> %d samples" % (numSamples, exSamples)
    start = 0
    length = 0
    for sample in range(numSamples - 1):
        ...
        #print "run: %d, start: %d, end %d" % (sample, start, end)
        print "%0.2f%%" % ((float(end) / float(exSamples)) * 100)
...
def perform_dpa(iPlaintext, iTraces, showPlot=True, fullPlot=False, silent=False):
    round_key = np.zeros(16)
    maxCorrs = np.zeros(256)

    #precalculations for corrcoef
    numtraces = len(iTraces)
    numSamples = iTraces.shape[1]

    Tmean = np.sum(iTraces, 0) / np.double(numtraces)
    Tdiff = iTraces - Tmean
    ...
    for bytenum in range(16):
        ...
        #H = hamming_s_box[np.tile(iPlaintext[:,bytenum], (256,1)).T^key.T]
        int1 = np.tile(iPlaintext[:,bytenum], (256,1)).T^key.T
        int2 = s_box[int1]
        H = hamming_weight_8bit_table[int1^int2]

        #comparing the hypothetical power consumption values with the power traces
        R = np.array(calccorrcoef(H, Tdiff, TdiffSumSquared, numtraces), np.double)
        Rabs = abs(R)
    ...
    if showPlot:
        ...
        for plotnum in range(16):
            ...
            ax.axis(limits)
        plt.savefig('results/keys-all.png')
        plt.savefig('results/keys-all.pdf', format='pdf')
        plt.show()
