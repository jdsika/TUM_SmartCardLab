...
void aes_invmixed_columns(uint8_t* s, uint8_t maxIndex)
{
	...
	for(i = 0; i < maxIndex / 4; ++i){
		...
		// 9 * A						= B
		v = aes_gfmul(0x09, v); 			// 0x1b strange modulo GF8

		// (4 * (a0 XOR a2)) XOR B		= C
		w = v ^ aes_gfmul(0x04, s[4*i+2] ^ s[4*i+0]);

		// (4 * (a1 XOR a3)) XOR B		= D
		v = v ^ aes_gfmul(0x04, s[4*i+3] ^ s[4*i+1]);

		// a3 XOR D XOR 2*(a0 XOR a3)	= b3
		s[4*i+3] = s[4*i+3] ^ v ^ aes_gfmul(0x02, s[4*i+0] ^ s[4*i+3]);

		// a1 XOR D XOR 2*(a1 XOR a2)	= b1
		s[4*i+1] = s[4*i+1] ^ v ^ aes_gfmul(0x02, s[4*i+2] ^ s[4*i+1]);

		// a2 XOR C XOR 2*(a3 XOR a2)	= b2
		s[4*i+2] = s[4*i+2] ^ w ^ aes_gfmul(0x02, t);

		// a0 XOR C XOR 2*(a0 XOR a1)
		s[4*i+0] = s[4*i+0] ^ w ^ aes_gfmul(0x02, u);
	}
